...
<title>Ajout de produit dans la base</title>
</head>
  ...
  if (isset($_POST['addProduct'])) {
    ...
    if ($uploadOk == 0) {
      ...
      if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
        if (ModelStocks::addProduct( $_POST['product'], $_POST['type'], $_FILES['photo']['name'], $_POST['description'] ))
        {
          ViewTemplates::alert("success", "Ajout de produit effectué avec succès", "productList.php");
        }
        else {
